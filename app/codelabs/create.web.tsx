...
export default function CodelabEditor() {
  ...
  const saveCodelab = async () => {
    ...
    try {
      ...
      console.log('Attempting to save codelab to Supabase with corrected schema:', JSON.stringify(dataToSave, null, 2));
      ...
      try {
        ...
        setError("Could not save to Supabase, but saved draft locally. Please check console logs.");
      } catch (localErr) {
        console.error('Failed fallback save to local storage:', localErr);
        setError("Failed to save codelab. Please try again and check console logs for details.");
      }
    }
  };
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.saveButtonContainer}>
        <Button title="Save Codelab" onPress={saveCodelab} color="#3b82f6" />
      </View>
    </ScrollView>
  );
}
